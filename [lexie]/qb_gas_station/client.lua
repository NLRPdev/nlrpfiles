...
RegisterNUICallback('renameMarket', function(data, cb)
	TriggerServerEvent('gas_station:renameMarket',empresaAtual,data)
end)
...
RegisterNUICallback('hideBalance', function(data, cb)
	TriggerServerEvent('gas_station:hideBalance',empresaAtual,data)
end)

RegisterNUICallback('showBalance', function(data, cb)
	TriggerServerEvent('gas_station:showBalance',empresaAtual,data)
end)
...
RegisterNUICallback('hirePlayer', function(data, cb)
	if cooldown == nil then
		cooldown = true
		local key = empresaAtual
		TriggerServerEvent('gas_station:hirePlayer',key,data.user)
		SetTimeout(500,function()
			cooldown = nil
		end)
	end
...
RegisterNUICallback('firePlayer', function(data, cb)
	if cooldown == nil then
		cooldown = true
		local key = empresaAtual
		TriggerServerEvent('gas_station:firePlayer',key,data.user)
		SetTimeout(500,function()
			cooldown = nil
		end)
	end
...
RegisterNUICallback('changeRole', function(data, cb)
	local key = empresaAtual
	TriggerServerEvent('gas_station:changeRole',key,data.user_id,data.role)
end)
...
RegisterNUICallback('giveComission', function(data, cb)
	if cooldown == nil then
		cooldown = true
		local key = empresaAtual
		TriggerServerEvent('gas_station:giveComission',key,data.user,data.amount)
		SetTimeout(500,function()
			cooldown = nil
		end)
	end
