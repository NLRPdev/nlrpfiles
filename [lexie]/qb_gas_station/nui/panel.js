	...
	if (item.notification){
		...
		*/
		...
		if (gas_station_business.gas_station_name) {
			$('#store-name').append(Lang[config.lang]['str_rename_store'] + ' <small>(' + gas_station_business.gas_station_name + ')</small>');
		} else {
			$('#store-name').append(Lang[config.lang]['str_rename_store']);
		}
		$('#store-name-desc').empty();
		$('#store-name-desc').append(Lang[config.lang]['str_rename_store_desc']);
		$('#form_blip_color').empty();
		$('#form_blip_color').append(Lang[config.lang]['html_select_colors']);
		$('#form_blip_id').empty();
		$('#form_blip_id').append(Lang[config.lang]['html_select_type']);
		$('#apply-rename').empty();
		$('#apply-rename').append(Lang[config.lang]['btn_apply_rename']);
		$('#text-rename').empty();
		$('#text-rename').append(`<input id="form_name" type="text" name="name" class="form-control" placeholder="${Lang[config.lang]['str_rename_store_name']}" required="required" oninput="InvalidMsg(this);" oninvalid="InvalidMsg(this);"> `);
		...
		*/
		...
		$('#input-hire-player').append(`<option value="" selected disabled>${Lang[config.lang]['select_employee']}</option>`);
				...
				<li class="d-flex justify-content-between">
					<div class="d-flex flex-row align-items-center"><img class="font-large-2 mr-2 " src="img/man.png" width="30">
						<div class="ml-2">
							...
							<div class="d-flex flex-row mt-1 text-black-50 small">
								<div><i class="fas fa-truck-loading"></i><span class="ml-2">${Lang[config.lang]['jobs_done']} ${user.jobs_done}</span></div>
								<div class="ml-3"><i class="fas fa-calendar-alt"></i><span class="ml-2">${Lang[config.lang]['hired_date']} ${timeConverter(user.timer,config.format.location)}</span></div>
							</div>
						</div>
					</div>
					<div class="d-flex align-self-center">
						<div>
							<select id="change_role" name="role" class="form-control" required="required">
								<option user="${user.user_id}" role="1" ${roles[1]??''}>${Lang[config.lang]['basic_access']}</option>
								<option user="${user.user_id}" role="2" ${roles[2]??''}>${Lang[config.lang]['advanced_access']}</option>
								<option user="${user.user_id}" role="3" ${roles[3]??''}>${Lang[config.lang]['full_access']}</option>
							</select>
						</div>
						<div class="d-flex flex-row align-items-center">
						...
						<div class="d-flex flex-row align-items-center">
							<button onclick="firePlayer('${user.user_id}')" class="btn btn-red btn-accent-4 white ml-3">${Lang[config.lang]['button_fire_employee']}</button>
						</div>
					<div>
				</li>
			`);
		...
		*/
		...
		$('#span-employee-page').append(Lang[config.lang]['str_employee_page_title_navbar']);
		...
		$('#employees-title').text(Lang[config.lang]['employees_title']);
		$('#employees-page-desc').text(Lang[config.lang]['employees_desc']);
		$('#button-hire-player').text(Lang[config.lang]['button_employee']);
		...
		$('#bank-page-show-hidden').append(Lang[config.lang]['str_bank_page_hidden_balance']);
...
function openPage(pageN){
	if (role >= config.roles_permissions[pageN]){
		$(".pages").css("display", "none");
...
function post(name,data){
	$.post("http://qb_gas_station/"+name,JSON.stringify(data),function(datab){
		if (datab != "ok"){
			console.log(datab);
		}
	});
