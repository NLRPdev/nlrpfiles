...
CreateThread(function()
	...
	for k, v in pairs(PresetList) do
		...
		exports['qb-target']:AddCircleZone(v["tex"].texn, v["coords"], v["radius"], { name=v["tex"].texn, debugPoly=Config.Debug, useZ=true, },
			{ options = { 
					{ type = "client", event = "jim-pizzathis:DuiSelect", icon = "fas fa-circle-check", label = "Add Image", job = JobGrade, tex = v["tex"].texn, size = v["tex"].size, texd = v["tex"].texd }, 
					{ type = "server", event = "jim-pizzathis:Server:ChangeDUI", icon = "fas fa-circle-minus", label = "Reset Image", job = JobGrade, tex = v["tex"].texn, texd = v["tex"].texd }, 
					{ type = "server", event = "jim-pizzathis:Server:ClearDUI", icon = "fas fa-circle-xmark", label = "Clear Image", job = JobGrade, tex = v["tex"].texn, texd = v["tex"].texd }, 
			}, distance = 4.0 })
	end
end)
...
RegisterNetEvent("jim-pizzathis:DuiSelect", function(data)
	...
	local dialog = exports['qb-input']:ShowInput({
        header = image.."Enter New Image URL",
        submitText = "Change",
...
RegisterNetEvent("jim-pizzathis:ChangeDUI", function(data)
	if Config.Debug then print("Making new DUI - "..data.url) end
