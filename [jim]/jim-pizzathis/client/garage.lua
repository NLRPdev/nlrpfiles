	...
	for k, v in pairs(Config.Locations) do job = v.label
		if v.garage then
			...
			exports['qb-target']:AddBoxZone("PizzaGarage: "..k, vector3(out.x, out.y, out.z-1.03), 0.8, 0.5, { name="PizzaGarage: "..k, heading = out[4]+180.0, debugPoly=Config.Debug, minZ=(out.z-1.03)-0.1, maxZ=out.z-1.03+1.3 }, 
				{ options = { { event = "jim-pizzathis:client:Garage:Menu", icon = "fas fa-clipboard", label = "Job Vehicles", job = job, coords = v.garage.spawn, list = v.garage.list }, },
				distance = 2.0 })
...
RegisterNetEvent('jim-pizzathis:client:Garage:Menu', function(data)
	...
	local vehicleMenu = { { icon = "fas fa-car-tunnel", header = "Job Garage", isMenuHeader = true } }	
	vehicleMenu[#vehicleMenu + 1] = { icon = "fas fa-circle-xmark", header = "", txt = "Close", params = { event = "jim-pizzathis:client:Menu:Close" } }
	if currentVeh.out and DoesEntityExist(currentVeh.current) then
		...
		vehicleMenu[#vehicleMenu+1] = { icon = "fas fa-car-burst", header = "Remove Vehicle", params = { event = "jim-pizzathis:client:RemSpawn" } }
	else
...
AddEventHandler('onResourceStop', function(r) if r ~= GetCurrentResourceName() then return end
	for k, v in pairs(Config.Locations) do if v.garage then exports['qb-target']:RemoveZone("PizzaGarage: "..k) end end
