...
CreateThread(function()
    ...
    exports['ghmattimysql']:execute('CREATE TABLE IF NOT EXISTS stores (id int AUTO_INCREMENT, name varchar(100), coordinate text, owner varchar(120), stock longtext, products longtext, request longtext, balance varchar(20), PRIMARY KEY(id))', {}, function()
        exports['ghmattimysql']:execute('SELECT * FROM stores', {}, function(result)
                ...
                for j,c in pairs(k.coordinates) do
                    ...
                    for l,m in pairs(server_stores) do
                        ...
                        if pos == pos2 then
                            ...
                            if m.name ~= i then
                                exports['ghmattimysql']:execute('UPDATE stores SET name = @name WHERE id = @id', {
                                    ['@id'] = l,
                                    ['@name'] = i
                                })
                    ...
                    if not found then
                        ...
                        exports['ghmattimysql']:execute('INSERT INTO stores (name, coordinate, owner, stock, products, request, balance) VALUES (@name, @coordinate, @owner, @stock, @products, @request, @balance)', {
                            ['@name'] = name, 
            ...
            for i,k in pairs(server_stores) do
				if(stores[k.name]) then
					...
					if changed then
                        exports['ghmattimysql']:execute('UPDATE stores SET products = @products, stock = @stock WHERE id = @id', {
                            ['@id'] = i,
