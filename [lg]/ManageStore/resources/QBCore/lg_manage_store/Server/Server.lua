...
AddEventHandler("lg_stores: SendBuyStore", function(name, position)
    ...
    if found_store and found_store.owner ~= "none" then
        ...
        exports['ghmattimysql']:execute('UPDATE stores SET owner = @owner WHERE id = @id', {
            ['@owner'] = identifier,
            ['@id'] = id_store
        })

        exports['ghmattimysql']:execute('SELECT * FROM stores WHERE id = @id', {
            ['@id'] = id_store
        }, function(result)
...
AddEventHandler("lg_stores: SendAddMoney", function(name, position, data)
    ...
    if found_store and found_store.owner == identifier then
        ...
        exports['ghmattimysql']:execute('UPDATE stores SET balance = @balance WHERE id = @id', {
...
AddEventHandler("lg_stores: SendChangePrice", function(name, position, data)
    ...
    if found_store and found_store.owner == identifier then
        ...
        exports['ghmattimysql']:execute('UPDATE stores SET products = @products WHERE id = @id', {
            ['@products'] = json.encode(found_store.products),
            ['@id'] = id_store
        })
...
AddEventHandler("lg_stores: SendRequestItem", function(name, position, data)
    ...
    if found_store and found_store.owner == identifier then
        ...
        exports['ghmattimysql']:execute('UPDATE stores SET request = @request, balance = @balance WHERE id = @id', {
...
AddEventHandler("lg_stores: CompleteMission", function(name, position)
    ...
    if found_store then
        ...
        exports['ghmattimysql']:execute('UPDATE stores SET request = @request, stock = @stock WHERE id = @id', {
            ['@request'] = json.encode(found_store.request),
            ['@stock'] = json.encode(found_store.stock),
            ['@id'] = id_store
        })
...
AddEventHandler("lg_stores: BuyItens", function(name, position, data)
    ...
    if found_store then
        ...
        if found_store.owner ~= 'none' then
            ...
            exports['ghmattimysql']:execute('UPDATE stores SET products = @products, balance = @balance WHERE id = @id', {
                ['@products'] = json.encode(found_store.products),
                ['@balance'] = found_store.balance,
                ['@id'] = id_store
            })
