    ...
    DropOffZone:onPlayerInOut(function(isPointInside, point)
        if isPointInside then
            ...
            if GetVehiclePedIsIn(PlayerPedId(), true) == boostVehicle and DropOffZone:isPointInside(GetEntityCoords(boostVehicle)) then
                ...
                TriggerEvent('qb-phone:client:CustomNotification', Translations.info['current'], Translations.info['park_vehicle_scratch'], 'fas fa-car', '#ff002f', 10000)
...
RegisterNetEvent('qb-phone:client:CustomNotification', function(title, text, icon, color, timeout)
    TriggerEvent('gksphone:notifi', {
        title = title,
        message = text,
        img = '/html/static/img/icons/mail.png'
    })
end)